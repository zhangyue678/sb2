...
def prepare_data(
    ...
    text_list = get_all_files(data_transformed, match_and=extension)
    if different_speakers:
        data_split = split_different_speakers(text_list)
    else:
        data_split = split_sets(text_list, split_ratio)
...
def create_json(text_list, json_file):
    """
    ...
    for text_file in text_list:
        ...
        path_parts = text_file.split(os.path.sep)
...
def transform_data(path_loadSession, path_structured_data):
    ...
    for k in range(5):
        ...
        logger.info("=" * 50)
        if save_textFile(session_, pathName) == 0:
            logger.info(
...
def load_session(pathSession):
    ...
    pathTextFolder = pathSession + "/dialog/transcriptions/"
    ...
    for emoFile in [
        ...
        for utterance in load_utterInfo(pathEmo + emoFile):
            if (
                ...
                id = utterance[2]
                path = pathTextFolder + emoFile
            
                with open(path, 'r') as f:
                    lines = f.readlines()
                for line in lines:
                    line = line.strip()
                    id_text = line.split(' ')[0]
                    if id==id_text:
                        text = line.split(':')[-1].strip()

                        if emoFile[7] != "i" and utterance[2][7] == "s":
                            improvisedUtteranceList.append(
                                [text, utterance[3], utterance[2][18]]
                            )
                        else:
                            improvisedUtteranceList.append(
                                [text, utterance[3], utterance[2][15]]
                            )
...
def save_textFile(session, pathName):
    """
    ...
    for idx, utterance in enumerate(session):
        ...
        makedirs(directory)
        filename = "%s/psn%s%s_%s_s%03d_orgn.txt" % (
            directory,
            pathName[-4],
            pathName[-2],
            label,
